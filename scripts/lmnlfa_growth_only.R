## lmnlfa_growth_only.R
## Stage 4/5 convergence check: full longitudinal item data (all waves),
## random intercept + random slope growth factor with their correlation
## (Omega[1,2]) -- but NO DIF and NO covariate impact on the growth factor.
##
## Purpose: isolate whether the intercept/slope correlation is identifiable
## from the repeated-measures item data alone, before adding covariate
## impact (age on the growth factor) or DIF back in. This reuses the exact
## lmnlfa-quad.stan model used by lmnlfa_hpc.R -- only the data passed in
## differs (ldf all zero => no DIF; nfpreds = 0 => no covariate impact) --
## so results are directly comparable to the full HPC run.
##
## Usage:
##   Rscript lmnlfa_growth_only.R <sex>
##   sex: female | male
##
## Stan:    scripts/stan/lmnlfa-quad.stan
## Outputs: written to <OUT_DIR>/lmnlfa_growth_only/
...
if (length(args) < 1) {
  stop("Usage: Rscript lmnlfa_growth_only.R <female|male>")
}
sx <- args[1]
...
out_dir <- file.path(out_base, "lmnlfa_growth_only")
...
# COMPILE STAN MODEL
# ---------------------------------------------------------------------------
# NOTE: scripts/stan/ is shared (sshfs) with the HPC cluster, which compiles
# its own lmnlfa-quad binary there (Linux ELF). To avoid clobbering that
# binary -- or running it and hitting "cannot execute binary file" on macOS --
# compile this run's executable into a local, isolated path instead of next
# to the .stan source.
bin_dir <- file.path(out_dir, "bin")
dir.create(bin_dir, showWarnings = FALSE, recursive = TRUE)
cat("\nCompiling Stan model (local exe, isolated from HPC binary)...\n")
stan_model <- cmdstan_model(
  stan_file,
  exe_file = file.path(bin_dir, "lmnlfa-quad-local"),
  force_recompile = TRUE
)
cat("Compiled.\n")

# ---------------------------------------------------------------------------
# HELPERS (identical to lmnlfa_hpc.R)
# ---------------------------------------------------------------------------
...
# FIT
# ---------------------------------------------------------------------------
cat("\n--- Fitting growth-only model (no DIF, no covariate impact) ---\n")
t0 <- proc.time()
fit <- tryCatch(
  {
    stan_model$sample(
...
fit$save_object(file.path(out_dir, paste0("fit_growth_only_", sx, ".rds")))

save_diagnostics(fit, paste0("growth_only_", sx), out_dir)
