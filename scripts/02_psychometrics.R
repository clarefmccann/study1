...
datasets <- list(
  female_parent = list(df = female_parent, sex = "female", reporter = "parent"),
  female_youth = list(df = female_youth, sex = "female", reporter = "youth"),
  male_parent = list(df = male_parent, sex = "male", reporter = "parent"),
  male_youth = list(df = male_youth, sex = "male", reporter = "youth")
)
...
for (ds_name in names(datasets)) {
  ...
  for (wv in wave_order) {
    ...
    # --- CFA 1-factor WLSMV ---
    cfa_syntax <- paste("pub =~", paste(ordinal_items, collapse = " + "))
    result$cfa <- tryCatch(
      lavaan::cfa(
        cfa_syntax,
        data = sub,
    ...
    grm_fit <- tryCatch(
      {
        mat <- as.matrix(sub) - 1L # mirt expects 0-based for graded
    ...
    if (!is.null(grm_fit)) {
      ...
      if (!is.null(coefs)) {
        ...
        # use base data.frame with explicit columns — avoids vctrs bind_rows issues
...
build_cr_data <- function(parent_df, youth_df) {
  parent_df %>%
    select(
      id,
      wave,
...
run_bifactor_section <- function(parent_df, youth_df, sex_label) {
  dat <- build_cr_data(parent_df, youth_df)
  cat(
  ...
  mod_2f <- '
    parent =~ peta_p + petb_p + petc_p + petd_p
    youth  =~ peta_y + petb_y + petc_y + petd_y
  '
  fit_2f <- tryCatch(
    lavaan::cfa(
      mod_2f,
  ...
  mod_bf <- '
    general  =~ peta_p + petb_p + petc_p + petd_p + peta_y + petb_y + petc_y + petd_y
    method_p =~ peta_p + petb_p + petc_p + petd_p
    method_y =~ peta_y + petb_y + petc_y + petd_y
  '
  fit_bf <- tryCatch(
    lavaan::cfa(
      mod_bf,
...
bf_female <- run_bifactor_section(female_parent, female_youth, "female")
bf_male <- run_bifactor_section(male_parent, male_youth, "male")
...
run_longitudinal_invariance <- function(df, label) {
  cat("\n\n=== Longitudinal invariance:", label, "===\n")

  dat <- df %>%
    select(wave, all_of(ordinal_items)) %>%
    filter(wave %in% wave_order) %>%
    mutate(
      wave = factor(wave, levels = wave_order),
      across(all_of(ordinal_items), ~ as.ordered(as.integer(.)))
    ) %>%
    filter(if_all(all_of(ordinal_items), ~ !is.na(.)))
  ...
  syntax <- paste("pub =~", paste(ordinal_items, collapse = " + "))
...
long_inv <- list(
  female_parent = run_longitudinal_invariance(female_parent, "female_parent"),
  female_youth = run_longitudinal_invariance(female_youth, "female_youth"),
  male_parent = run_longitudinal_invariance(male_parent, "male_parent"),
  male_youth = run_longitudinal_invariance(male_youth, "male_youth")
)
...
run_covariate_invariance <- function(df, covariate, group_label, ds_label) {
  dat <- df %>%
    ...
    filter(if_all(all_of(ordinal_items), ~ !is.na(.))) %>%
    select(group_var, all_of(ordinal_items))
...
for (ds_name in names(datasets)) {
  ...
  for (cov in c("age_tertile", "bmi_tertile", "race_group", "site_group")) {
    res <- run_covariate_invariance(df_aug, cov, cov, ds_name)
...
# ---------------------------------------------------------------------------
if (nrow(disc_table) > 0) {
  p_disc <- ggplot(disc_table, aes(x = wave, y = item, fill = a)) +
    geom_tile(colour = "white") +
    geom_text(aes(label = round(a, 2)), size = 3) +
    scale_fill_gradient2(
      low = "#d73027",
      mid = "#fee090",
      high = "#1a9850",
      midpoint = 1,
      name = "a (discrimination)"
    ) +
    facet_grid(reporter ~ sex) +
    labs(
      title = "GRM item discrimination by wave, reporter, and sex",
      x = "Wave",
      y = "Item"
    ) +
