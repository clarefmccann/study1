...
if (length(args) < 1) {
  stop("Usage: Rscript lmnlfa_hpc.R <female|male>")
}
sx <- args[1]
...
extract_factor_scores <- function(fit2, prep, ldf_step2, sex_label) {
  ...
  b_phi_mean <- colMeans(as.matrix(draws[,
    grep("^b_phi\\[", names(draws)),
    drop = FALSE
  ]))
  ...
  L_Omega_mean <- colMeans(as.matrix(draws[, grep(
    "^L_Omega\\[",
    names(draws)
  )]))
...
extract_item_params <- function(
  ...
  if (length(dif_item_idx) > 0) {
    ...
    if (length(ldiftv_cols) > 0) {
      ldiftv_mean[dif_item_idx] <- colMeans(as.matrix(draws_dif[,
        ldiftv_cols,
        drop = FALSE
      ]))
    }
    if (length(ndiftv_cols) > 0) {
      ndiftv_mean[dif_item_idx] <- colMeans(as.matrix(draws_dif[,
        ndiftv_cols,
        drop = FALSE
      ]))
    }
...
if (file.exists(rds_fit1)) {
  ...
  fit1 <- tryCatch(
    ...
    error = function(e) {
      cat("\n!!! Step 1 sampling failed:", conditionMessage(e), "\n")
      csv_files <- list.files(
        out_dir,
        pattern = "^lmnlfa-quad.*\\.csv$",
        full.names = TRUE
      )
      for (f in csv_files) {
  ...
  cat(
    "Step 1 elapsed:",
    round((proc.time() - t0)[["elapsed"]] / 3600, 2),
    "hr\n"
  )
...
scores <- tryCatch(
  ...
  error = function(e) {
    message("Factor scores failed: ", e$message)
    NULL
  }
)
if (!is.null(scores)) {
  ...
  set.seed(90025)
  samp_ids <- sample(
    unique(scores$person_idx),
    min(200, length(unique(scores$person_idx)))
  )
  p_scores <- ggplot(
  ...
  ggsave(
    file.path(out_dir, paste0("factor_score_trajectories_", sx, ".png")),
    p_scores,
    width = 8,
    height = 5,
    dpi = 180
...
write.csv(
  ip,
  file.path(out_dir, paste0("item_params_", sx, ".csv")),
  row.names = FALSE
)
