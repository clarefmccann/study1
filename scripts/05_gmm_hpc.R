...
for (k in 2:max_k) {
  ...
  if (file.exists(rds_k)) {
    ...
    if (!is.null(models[[k]])) {
      ...
      cat(
        "  loglik:",
        round(models[[k]]$loglik, 2),
        " BIC:",
        round(models[[k]]$BIC, 2),
        " entropy:",
        round(calc_entropy(models[[k]]), 3),
        "\n"
      )
...
cat("\n=== Model fit indices ===\n")
print(
  fit_table[, c("K", "loglik", "BIC", "entropy", "avepp", "class_pct")],
  digits = 3,
  row.names = FALSE
)
...
p_sel <- ggplot(fit_long, aes(x = K, y = value)) +
  ...
  labs(
    ...
    subtitle = paste0(
      n_starts,
      " random starts per K; quadratic trajectory, nwg=FALSE"
    ),
...
ggsave(
  file.path(out_dir, paste0(ds_name, "_gmm_model_selection.png")),
  p_sel,
  width = 9,
  height = 4,
  dpi = 180
...
for (m in models) {
  ...
  p_traj <- ggplot(traj, aes(x = age, y = pds_pred, colour = class)) +
    ...
    scale_colour_manual(
      values = setNames(pal[seq_len(k)], paste0("Class ", seq_len(k))),
      labels = lbls
    ) +
